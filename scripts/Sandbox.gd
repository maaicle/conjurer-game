...
func _ready() -> void:
	...
	toolbar_ui = ToolbarUI.new($UI/Toolbar)
	toolbar_ui.action_pressed.connect(_on_action_button_pressed)
	turn_manager = TurnManager.new(self)
	turn_manager.selection_changed.connect(_on_unit_selection_changed)
	grid_highlighter = GridHighlighter.new(ground_layer, self)
	base_placement = BasePlacementController.new(tile_grid, grid_highlighter, self)
	base_placement.avatar_placed.connect(_on_avatar_placed)
	base_placement.state_changed.connect(update_toolbar)
	movement = MovementController.new(tile_grid, turn_manager, base_placement, grid_highlighter, self)
	movement.state_changed.connect(update_toolbar)
	spawn_controller = SpawnController.new(tile_grid, turn_manager, base_placement, grid_highlighter, self)
	spawn_controller.state_changed.connect(update_toolbar)
	spawn_controller.spawn_started.connect(movement.exit_move_mode)
	spawn_controller.action_completed.connect(movement.enter_move_mode)
	draw_grid_labels()
	fit_camera_to_map()
	update_toolbar()
